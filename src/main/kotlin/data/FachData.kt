...
class FachData(
    ...
    fun filterWahlzeilen(lk1: Fach?, lk2: Fach?, pf3: Fach?, pf4: Fach?, pf5: Fach?): Map<Int, Wahlzeile> {
        ...
        if (lk1 != null) predicates.add { it.lk1 in wildcardMapping[lk1]!!}
        if (lk2 != null) predicates.add { it.lk2 in wildcardMapping[lk2]!!}
        if (pf3 != null) predicates.add { it.pf3 in wildcardMapping[pf3]!!}
        if (pf4 != null) predicates.add { it.pf4 in wildcardMapping[pf4]!!}
        if (pf5 != null) predicates.add { it.pf5 in wildcardMapping[pf5]!!}

        if (predicates.isEmpty()) return wahlzeilen

        return wahlzeilen.filterValues { wz -> predicates.all { it(wz) } }
    }
